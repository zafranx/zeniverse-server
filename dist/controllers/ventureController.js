...
exports.deleteVenture = exports.updateVenture = exports.createVenture = exports.createVenture22 = exports.createVenturexxxx = exports.getVentureById = exports.getAllVentures = void 0;
...
const getAllVentures = async (req, res) => {
    try {
        ...
        const [ventures, total] = await Promise.all([
            Ventures_1.default.find(filter)
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .lean(),
            Ventures_1.default.countDocuments(filter),
        ]);
...
exports.getAllVentures = getAllVentures;
const getVentureById = async (req, res) => {
    try {
        const venture = await Ventures_1.default.findById(req.params.id).lean();
        if (!venture) {
            res
                .status(constants_1.RESPONSE_CODES.NOT_FOUND)
                .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.NOT_FOUND, "Venture not found"));
            return;
        }
        res
...
exports.getVentureById = getVentureById;
const createVenturexxxx = async (req, res) => {
    try {
        const body = req.body;
        // Handle uploads (Cloudinary/local)
        const uploaded = await (0, multer_1.processUploadedFiles)(req, req.files);
        if (uploaded.logo)
            body.logo = uploaded.logo;
        if (uploaded.bannerImage)
            body.bannerImage = uploaded.bannerImage;
        // Arrays of bullets
        body.problemStatement = safeParseJSON(body.problemStatement, []).filter(Boolean);
        body.proposedSolutions = safeParseJSON(body.proposedSolutions, []).filter(Boolean);
        body.innovationValue = safeParseJSON(body.innovationValue, []).filter(Boolean);
        body.targetGeography = safeParseJSON(body.targetGeography, []).filter(Boolean);
        body.valueProposition = safeParseJSON(body.valueProposition, []).filter(Boolean);
        body.marketFitment = safeParseJSON(body.marketFitment, []).filter(Boolean);
        body.goToMarketStrategy = safeParseJSON(body.goToMarketStrategy, []).filter(Boolean);
        ...
        // Projections
        body.financialProjections = safeParseJSON(body.financialProjections, []);
        // Investors (JSON array or names + uploaded investorLogos[])
        let currentInvestors = safeParseJSON(body.currentInvestors, []);
        if (!currentInvestors.length) {
            const investorNames = safeParseJSON(body.currentInvestorsNames, []);
            const investorLogoFiles = req.files?.investorLogos || [];
            currentInvestors = investorNames.map((name, idx) => ({
                name,
        ...
        let founders = safeParseJSON(body.founders, []);
        if (!founders.length) {
            const founderNames = safeParseJSON(body.founderNames, []);
            const founderBios = safeParseJSON(body.founderBios, []);
            const founderPicFiles = req.files?.founderPics || [];
            founders = founderNames.map((name, idx) => ({
                name,
        ...
        const screenshotsFromJson = safeParseJSON(body.productScreenshots, []);
        const screenshotFiles = req.files?.productScreenshots || [];
        const screenshotsFromFiles = screenshotFiles.map((f) => f.path);
        ...
        const venture = new Ventures_1.default(body);
        await venture.save();
        res
            .status(constants_1.RESPONSE_CODES.CREATED)
            .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.CREATED, "Venture created successfully", { venture }));
    }
    catch (error) {
...
exports.createVenturexxxx = createVenturexxxx;
// src/controllers/ventureController.ts
const createVenture22 = async (req, res) => {
    try {
        const body = req.body;
        ...
        if (req.files) {
            ...
            if (files.investorLogos) {
                const investorNames = safeParseJSON(body.currentInvestorsNames, []);
                body.currentInvestors = investorNames.map((name, index) => ({
                    name: name.trim(),
            ...
            if (files.founderPics) {
                const founderNames = safeParseJSON(body.founderNames, []);
                const founderBios = safeParseJSON(body.founderBios, []);
                body.founders = founderNames.map((name, index) => ({
                    name: name.trim(),
            ...
            if (files.productScreenshots) {
                body.productScreenshots = files.productScreenshots.map((file) => getFilePath(file)).filter(Boolean);
            }
        }
        ...
        if (missing.length > 0) {
            res.status(constants_1.RESPONSE_CODES.VALIDATION_ERROR).json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.VALIDATION_ERROR, `Missing required fields: ${missing.join(', ')}`));
            return;
        }
        ...
        const venture = new Ventures_1.default(body);
        await venture.save();
        res.status(constants_1.RESPONSE_CODES.CREATED).json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.CREATED, 'Venture created successfully', { venture }));
    }
    catch (error) {
        console.error('Create venture error:', error);
...
exports.createVenture22 = createVenture22;
// src/controllers/ventureController.ts
const createVenture = async (req, res) => {
    try {
        console.log('Received venture data:', req.body); // Debug log
        const { ventureName, brandName, logo, // Now a URL string
        shortDescription, problemStatement, proposedSolutions, innovationValue, targetGeography, valueProposition, marketFitment, goToMarketStrategy, currentStatus, financialProjections, currentInvestors, // Array with logo URLs
        founders, // Array with pic URLs
        productScreenshots, // Array of URL strings
        bannerImage, // URL string
        bannerHeading, bannerText, } = req.body;
        // Create venture with URL strings directly
        const newVenture = new Ventures_1.default({
            ventureName,
            brandName,
        ...
        res.status(constants_1.RESPONSE_CODES.SUCCESS).json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.SUCCESS, constants_1.RESPONSE_MESSAGES.SUCCESS, { venture: newVenture }));
    }
    catch (error) {
        console.error('Create venture error:', error);
        res.status(constants_1.RESPONSE_CODES.INTERNAL_SERVER_ERROR).json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.INTERNAL_SERVER_ERROR, constants_1.RESPONSE_MESSAGES.INTERNAL_ERROR));
...
const updateVenture = async (req, res) => {
    try {
        ...
        const existing = await Ventures_1.default.findById(id);
        if (!existing) {
            res
                .status(constants_1.RESPONSE_CODES.NOT_FOUND)
                .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.NOT_FOUND, "Venture not found"));
        ...
        if (typeof body.problemStatement === "string")
            body.problemStatement = safeParseJSON(body.problemStatement, []);
        if (typeof body.proposedSolutions === "string")
            body.proposedSolutions = safeParseJSON(body.proposedSolutions, []);
        if (typeof body.innovationValue === "string")
            body.innovationValue = safeParseJSON(body.innovationValue, []);
        if (typeof body.targetGeography === "string")
            body.targetGeography = safeParseJSON(body.targetGeography, []);
        if (typeof body.valueProposition === "string")
            body.valueProposition = safeParseJSON(body.valueProposition, []);
        if (typeof body.marketFitment === "string")
            body.marketFitment = safeParseJSON(body.marketFitment, []);
        if (typeof body.goToMarketStrategy === "string")
            body.goToMarketStrategy = safeParseJSON(body.goToMarketStrategy, []);
        ...
        if (typeof body.currentInvestors === "string") {
            body.currentInvestors = safeParseJSON(body.currentInvestors, []);
        }
        else if (body.currentInvestorsNames) {
            const investorNames = safeParseJSON(body.currentInvestorsNames, []);
            const investorLogoFiles = req.files?.investorLogos || [];
            body.currentInvestors = investorNames.map((name, idx) => ({
                name,
                logo: investorLogoFiles[idx]?.path || "",
            }));
        }
        if (typeof body.founders === "string") {
            body.founders = safeParseJSON(body.founders, []);
        }
        else if (body.founderNames || body.founderBios) {
            const founderNames = safeParseJSON(body.founderNames, []);
            const founderBios = safeParseJSON(body.founderBios, []);
            const founderPicFiles = req.files?.founderPics || [];
            body.founders = founderNames.map((name, idx) => ({
                name,
                briefBio: founderBios[idx] || "",
                pic: founderPicFiles[idx]?.path || "",
            }));
        ...
        const venture = await Ventures_1.default.findByIdAndUpdate(id, body, {
            new: true,
            runValidators: true,
        });
        res
            .status(constants_1.RESPONSE_CODES.SUCCESS)
            .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.SUCCESS, "Venture updated successfully", { venture }));
    }
    catch (error) {
...
const deleteVenture = async (req, res) => {
    try {
        const venture = await Ventures_1.default.findByIdAndDelete(req.params.id);
        if (!venture) {
            res
                .status(constants_1.RESPONSE_CODES.NOT_FOUND)
                .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.NOT_FOUND, "Venture not found"));
        ...
        res
            .status(constants_1.RESPONSE_CODES.SUCCESS)
            .json((0, constants_1.__requestResponse)(constants_1.RESPONSE_CODES.SUCCESS, "Venture deleted successfully"));
    }
    catch (error) {
...
//# sourceMappingURL=ventureController.js.map